  ...
  describe("separateAmPmPicker", () => {
    ...
    it("greys hour rows whose 24h value falls outside hourLimit (given current AM/PM)", () => {
      ...
      const isGreyed = el => {
        ...
        return arr.some(s => s && s.opacity === 0.2);
      };
    ...
    describe("initial-value round-trip", () => {
      it.each([[0, "12 AM"], [1, "1 AM"], [11, "11 AM"], [12, "12 PM"], [13, "1 PM"], [17, "5 PM"], [23, "11 PM"]])("emits hours=%i (%s) through onDurationChange", hours => {
        const onDurationChange = jest.fn();
        render(/*#__PURE__*/React.createElement(TimerPicker, {
          initialValue: {
            hours
        ...
        expect(onDurationChange).toHaveBeenLastCalledWith(expect.objectContaining({
          hours
        }));
      });
    });
    describe("setValue ref API", () => {
      it.each([[0], [1], [11], [12], [13], [17], [23]])("setValue({ hours: %i }) emits hours=%i", hours => {
        const onDurationChange = jest.fn();
        ...
        act(() => {
          var _ref$current;
          (_ref$current = ref.current) === null || _ref$current === void 0 || _ref$current.setValue({
            hours
          });
        });
        expect(onDurationChange).toHaveBeenLastCalledWith(expect.objectContaining({
    ...
    describe("ignored prop warnings", () => {
      ...
      it("warns when maximumHours is set with separateAmPmPicker", () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('"maximumHours" is currently ignored'));
      });
      it("does not warn when maximumHours is the default", () => {
        ...
        expect(warnSpy).not.toHaveBeenCalledWith(expect.stringContaining('"maximumHours" is currently ignored'));
      });
      it("does not warn about hourLimit anymore (it now works in separate mode)", () => {
        ...
        expect(warnSpy).not.toHaveBeenCalledWith(expect.stringContaining('"hourLimit" is currently ignored'));
      });
