...
const renderItem = props => render(/*#__PURE__*/React.createElement(PickerItem, {
...
const flattenStyle = element => {
  const style = element.props.style;
  if (!style) return [];
  return (Array.isArray(style) ? style : [style]).filter(Boolean);
};
const isDisabledStyle = element => flattenStyle(element).some(s => s.opacity === 0.3);
const isSelectedStyle = element => flattenStyle(element).some(s => s.fontWeight === "bold");
describe("PickerItem", () => {
  ...
  describe("12-hour cycle picker (separate AM/PM)", () => {
    ...
    describe("greying via isItemDisabled callback", () => {
      it("calls isItemDisabled with the parsed hourSlot ('12' → 0)", () => {
        const isItemDisabled = jest.fn(value => value === 0);
        const {
          getByText
        } = renderItem({
          adjustedLimitedMax: 11,
  ...
  describe("AM/PM picker", () => {
    ...
    describe("custom AM/PM styling", () => {
      ...
      const renderWithCustom = props => render(/*#__PURE__*/React.createElement(PickerItem, {
        adjustedLimitedMax: props.adjustedLimitedMax,
        adjustedLimitedMin: props.adjustedLimitedMin,
        allowFontScaling: false,
        amLabel: props.amLabel ?? "AM",
      ...
      it("applies selectedSeparateAmPmItem only to the selected AM/PM row", () => {
        ...
        const selectedFlat = getSelected("AM").props.style.flat();
        expect(selectedFlat.some(s => s && s.color === "tomato")).toBe(true);
        const {
          getByText: getUnselected
        } = renderWithCustom({
          adjustedLimitedMax: 1,
          adjustedLimitedMin: 0,
          isAmPmPicker: true,
        ...
        const unselectedFlat = getUnselected("PM").props.style.flat();
      ...
      it("does not apply separateAmPmItem to non-AM/PM rows", () => {
        ...
        const flat = getByText("05").props.style.flat();
