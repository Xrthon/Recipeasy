...
const resolveColumnWidth = (pickerColumnWidth, column) => {
  if (pickerColumnWidth == null) {
    return _styles.DEFAULT_COLUMN_WIDTH;
  }
  if (typeof pickerColumnWidth === "number") {
    return pickerColumnWidth;
  }
  return pickerColumnWidth[column] ?? _styles.DEFAULT_COLUMN_WIDTH;
};
const TimerPickerModal = /*#__PURE__*/(0, _react.forwardRef)((props, ref) => {
  ...
  const hasSeparateAmPmColumn = Boolean(otherProps.use12HourPicker && otherProps.separateAmPmPicker && !hideHours);
  const totalColumnWidth = (!hideDays ? resolveColumnWidth(pickerColumnWidth, "days") : 0) + (!hideHours ? resolveColumnWidth(pickerColumnWidth, "hours") : 0) + (!hideMinutes ? resolveColumnWidth(pickerColumnWidth, "minutes") : 0) + (!hideSeconds ? resolveColumnWidth(pickerColumnWidth, "seconds") : 0) + (hasSeparateAmPmColumn ? resolveColumnWidth(pickerColumnWidth, "amPm") : 0);
